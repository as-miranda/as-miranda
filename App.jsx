...
function Curriculum() {
  ...
  return <div className="page-shell"><PageIntro number="04" label="CURRICULUM" title={<>A RECORD OF<br/><em>THE WORK.</em></>} text="Um currículo que mostra trajetória, não apenas uma lista de cargos."/>
    <section className="cv-toolbar"><div><button className={mode==="timeline"?"selected":""} onClick={()=>setMode("timeline")}><Grid2X2 size={15}/> TIMELINE</button><button className={mode==="classic"?"selected":""} onClick={()=>setMode("classic")}><FileText size={15}/> CLASSIC</button></div><a className="button-primary" href="/CV_placeholder.pdf" download><FileText size={15}/> DOWNLOAD PDF</a></section>
